...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Residential','Commercial'], yticklabels=['Residential','Commercial'])
...
indices = random.sample(range(X_val.shape[0]), min(6, X_val.shape[0]))
...
for i, idx in enumerate(indices):
    ...
    plt.title(f"Pred: {'Commercial' if preds[i]==1 else 'Residential'}\nTrue: {'Commercial' if y_val[idx]==1 else 'Residential'}")
    plt.axis('off')
